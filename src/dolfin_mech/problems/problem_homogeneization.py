...
"""Defines the HomogenizationProblem class.

Implements a periodic homogenization
framework to compute the effective (homogenized) elastic properties of a
microstructure (REV). It solves corrector problems under unit macroscopic strains
to determine the full stiffness tensor and effective bulk moduli.
"""

import dolfin
import numpy

from ..core import PeriodicSubDomain

#############################################################################


class Homogenization:
	r"""Class for solving periodic homogenization problems in linear elasticity.
	...
	def __init__(self, dim, mesh, mat_params, vol, bbox, vertices=None):
	...
	def sigma(self, v, eps):
		r"""Computes the micro-stress tensor using the constitutive law.
		...
		"""
		return self.lmbda_s * dolfin.tr(eps + self.eps(v)) * dolfin.Identity(self.dim) + 2 * self.mu_s * (
			eps + self.eps(v)
		)
	...
	def Voigt2strain(self, s):
		...
		if self.dim == 2:
			return numpy.array([[s[0], s[2] / 2.0], [s[2] / 2.0, s[1]]])
		if self.dim == 3:
			return numpy.array([[s[0], s[5] / 2.0, s[4] / 2], [s[5] / 2.0, s[1], s[3] / 2], [s[4] / 2, s[3] / 2, s[2]]])
	...
	def get_lambda_and_mu(self):
		r"""Computes the homogenized Lamé coefficients by solving corrector problems.
		...
		W = dolfin.FunctionSpace(
			...
			constrained_domain=PeriodicSubDomain(self.dim, self.bbox, self.vertices),
		)
	...
	def get_kappa(self):
		...
		vertices = numpy.array([[xmin, ymin], [xmax, ymin], [xmax, ymax], [xmin, ymax]])
		...
		dS = dolfin.Measure("exterior_facet", domain=self.mesh, subdomain_data=boundaries_mf)
		...
		W = dolfin.FunctionSpace(
			self.mesh,
			dolfin.MixedElement([Ve, Re]),
			constrained_domain=PeriodicSubDomain(self.dim, self.bbox, vertices),
		)
