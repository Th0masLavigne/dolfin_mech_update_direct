...
class PoroHyperelasticity(Hyperelasticity):
	r"""General problem class for finite strain poro-hyperelasticity.
	...
	**Porosity Formulations:**
	...
	def __init__(
		...
		r"""Initializes the PoroHyperelasticityProblem.
		...
		self.init_known_porosity(porosity_init_val=porosity_init_val, porosity_init_fun=porosity_init_fun)
	...
	def set_known_and_unknown_porosity(self, porosity_known):
		r"""Sets the porosity variable strategy.
	...
	def init_known_porosity(self, porosity_init_val, porosity_init_fun):
		r"""Initializes the known porosity field using a constant value or a function."""
	...
	def add_porosity_subsol(self, degree, init_val=None, init_fun=None):
		r"""Adds the unknown porosity field to the solution space.
		...
		if degree == 0:
			self.porosity_subsol = self.add_scalar_subsol(
				name=self.porosity_unknown, family="DG", degree=0, init_val=init_val, init_fun=init_fun
			)
		else:
			self.porosity_subsol = self.add_scalar_subsol(
				name=self.porosity_unknown, family="CG", degree=degree, init_val=init_val, init_fun=init_fun
			)

	def add_pressure_balancing_gravity_subsol(self, degree=1):
		r"""Adds a sub-solution for the pressure field required to balance gravity."""
		self.pressure_balancing_gravity_subsol = self.add_scalar_subsol(
			name="pressure_balancing_gravity", family="CG", degree=degree
		)

	def add_lmbda_subsol(self, init_val=None):
		r"""Adds a global Lagrange multiplier (vector) for translation constraints."""
		self.lmbda_subsol = self.add_vector_subsol(name="lmbda", family="R", degree=0, init_val=init_val)

	def add_mu_subsol(self, init_val=None):
		r"""Adds a global Lagrange multiplier (vector) for rotation/moment constraints."""
		self.mu_subsol = self.add_vector_subsol(name="mu", family="R", degree=0, init_val=init_val)

	def add_gamma_subsol(self):
		r"""Adds a global Lagrange multiplier (scalar) for volume/pressure constraints."""
		self.gamma_subsol = self.add_scalar_subsol(name="gamma", family="R", degree=0)

	def get_deformed_center_of_mass(self):
		r"""Computes the center of mass in the current configuration based on the known porosity distribution."""
	...
	def add_deformed_center_of_mass_subsol(self):
		r"""Adds a global variable tracking the deformed center of mass."""
		self.deformed_center_of_mass_subsol = self.add_vector_subsol(
			name="xg", family="R", degree=0, init_val=self.get_deformed_center_of_mass()
		)

	def set_subsols(self, displacement_degree=1, porosity_degree=None, porosity_init_val=None, porosity_init_fun=None):
		...
		self.add_porosity_subsol(degree=porosity_degree, init_val=porosity_init_val, init_fun=porosity_init_fun)
	...
	def add_local_porosity_fois(self):
		...
		self.add_foi(expr=1.0 - self.Phis0, fs=self.porosity_subsol.fs.collapse(), name="Phif0")

		if self.porosity_known == "phis":
			self.add_foi(expr=self.Phis, fs=self.porosity_subsol.fs.collapse(), name="Phis")
		self.add_foi(expr=self.kinematics.J - self.Phis, fs=self.porosity_subsol.fs.collapse(), name="Phif")
	...
	def add_Wskel_operator(self, material_parameters, material_scaling, subdomain_id=None):
		r"""Adds the operator for the solid skeleton strain energy."""
		operator = operators.poro.Wskel(
	...
	def add_Wskel_operators(self, skel_behaviors):
		r"""Adds multiple skeleton operators and registers their specific stress FOIs."""
	...
	def add_Wbulk_operator(self, material_parameters, material_scaling, subdomain_id=None):
		r"""Adds the operator for the bulk mixture compressibility."""
		operator = operators.poro.Wbulk(
			kinematics=self.kinematics,
			U_test=self.displacement_subsol.dsubtest,
			Phis0=self.Phis0,
			Phis=self.Phis,
	...
	def add_Wbulk_operators(self, bulk_behaviors):
		r"""Adds multiple bulk operators and registers compressibility FOIs."""
		for bulk_behavior in bulk_behaviors:
			...
			self.add_foi(
				expr=operator.material.dWbulkdPhis * self.kinematics.J * self.kinematics.C_inv,
				fs=self.mfoi_fs,
				name="Sigma_bulk" + suffix,
			)
			self.add_foi(
				expr=operator.material.dWbulkdPhis * self.kinematics.I, fs=self.mfoi_fs, name="sigma_bulk" + suffix
			)

	def add_Wpore_operator(self, material_parameters, material_scaling, subdomain_id=None):
		r"""Adds the operator for pore contributions (e.g. surface tension, pressure potentials)."""
		operator = operators.poro.Wpore(
			kinematics=self.kinematics,
			Phis0=self.Phis0,
			Phis=self.Phis,
			unknown_porosity_test=self.porosity_subsol.dsubtest,
	...
	def add_Wpore_operators(self, pore_behaviors):
		r"""Adds multiple pore behavior operators."""
	...
	def add_pf_operator(self, k_step=None, **kwargs):
		r"""Adds an operator to define the fluid pressure field `pf` and registers it as a FOI."""
		operator = operators.poro.Pf(unknown_porosity_test=self.porosity_subsol.dsubtest, **kwargs)
		self.add_operator(operator=operator, k_step=k_step)
		self.add_foi(expr=operator.pf, fs=self.sfoi_fs, name="pf")

	def add_pressure_balancing_gravity_loading_operator(self, k_step=None, **kwargs):
		r"""Adds the operator solving for the pressure field that balances gravity loads."""
		operator = operators.loading.PressureBalancingGravity(
			X=self.X,
	...
	def add_global_porosity_qois(self):
		r"""Registers Quantities of Interest (QOI) for integrated porosity metrics.
		...
		self.add_qoi(name="Phif0", expr=(1.0 - self.Phis0) * self.dV)

		self.add_qoi(name="Phis", expr=self.Phis * self.dV)

		self.add_qoi(name="Phif", expr=(self.kinematics.J - self.Phis) * self.dV)

		self.add_qoi(name="phis", expr=self.phis * self.dV)

		self.add_qoi(name="phif", expr=(1.0 - self.phis) * self.dV)

	def add_global_stress_qois(self, stress_type="cauchy"):
		r"""Registers component-wise global stress QOIs (integrated over domain).
		...
		for compname, comp in zip(compnames, comps):
			if stress == "Sigma":
				self.add_qoi(
					...
					expr=sum(
						[
							getattr(operator.material, stress)[comp] * operator.measure
							for operator in self.operators
							if (hasattr(operator, "material") and hasattr(operator.material, stress))
						]
					),
				)
				self.add_qoi(
					...
					expr=sum(
						[
							getattr(operator.material, "dWbulkdPhis")
							* self.kinematics.J
							* self.kinematics.C_inv[comp]
							* operator.measure
							for operator in self.operators
							if (hasattr(operator, "material") and hasattr(operator.material, "dWbulkdPhis"))
						]
					),
				)
				self.add_qoi(
					...
					[
						getattr(operator.material, "dWbulkdPhis")[comp]
						* self.kinematics.J
						* self.kinematics.C_inv
						* operator.measure
						for operator in self.operators
						if (hasattr(operator, "material") and hasattr(operator.material, "dWbulkdPhis"))
					]
				...
				self.add_qoi(
					...
					expr=sum(
						[
							getattr(operator.material, stress)[comp] * self.kinematics.J * operator.measure
							for operator in self.operators
							if (hasattr(operator, "material") and hasattr(operator.material, stress))
						]
					),
	...
	def add_global_fluid_pressure_qoi(self):
		r"""Registers global fluid pressure QOI by summing contributions from all operators containing 'pf'."""
		# for operator in self.operators:
		#     print(type(operator))
		...
		self.add_qoi(
			...
			expr=sum(
				[
					operator.pf * operator.measure
					for step in self.steps
					for operator in step.operators
					if hasattr(operator, "pf")
				]
			),
		)
