...
class PeriodicSubDomain(dolfin.SubDomain):
	...
	**Mapping Logic:**
	...
	def __init__(self, dim, bbox, vertices=None, tol=None):
		...
		if self.dim == 2:
			...
			assert 0, "dim must be 2 or 3. Aborting."
		...
		assert (
			np.linalg.norm(self.vv[2, :] - self.vv[3, :] - self.a1) <= self.tol
		)  # check if UC vertices form indeed a parallelogram
		assert (
			np.linalg.norm(self.vv[2, :] - self.vv[1, :] - self.a2) <= self.tol
		)  # check if UC vertices form indeed a parallelogram
	...
	def inside_2D(self, x, on_boundary):
		...
		return bool(
			(
				dolfin.near(x[0], self.vv[0, 0] + x[1] * self.a2[0] / self.vv[3, 1], self.tol)
				or dolfin.near(x[1], self.vv[0, 1] + x[0] * self.a1[1] / self.vv[1, 0], self.tol)
			)
			and (
				not (
					(dolfin.near(x[0], self.vv[1, 0], self.tol) and dolfin.near(x[1], self.vv[1, 1], self.tol))
					or (dolfin.near(x[0], self.vv[3, 0], self.tol) and dolfin.near(x[1], self.vv[3, 1], self.tol))
				)
			)
			and on_boundary
		)
	...
	def inside_3D(self, x, on_boundary):
		...
		return bool(
			...
			and not (
				dolfin.near(x[0], self.xmax, self.tol)
	...
	def map_2D(self, x, y):
		...
		"""
		if dolfin.near(x[0], self.vv[2, 0], self.tol) and dolfin.near(
	...
	def map_3D(self, x, y):
		...
		# Edges
			...
			(dolfin.near(x[0], self.xmax, self.tol) and dolfin.near(x[1], self.ymax, self.tol))
			or (dolfin.near(x[0], self.xmax, self.tol) and dolfin.near(x[1], self.ymin, self.tol))
			or (dolfin.near(x[0], self.xmin, self.tol) and dolfin.near(x[1], self.ymax, self.tol))
		):
			y[0] = self.xmin
			y[1] = self.ymin
			y[2] = x[2]
		elif (
			(dolfin.near(x[0], self.xmax, self.tol) and dolfin.near(x[2], self.zmax, self.tol))
			or (dolfin.near(x[0], self.xmax, self.tol) and dolfin.near(x[2], self.zmin, self.tol))
			or (dolfin.near(x[0], self.xmin, self.tol) and dolfin.near(x[2], self.zmax, self.tol))
		):
			y[0] = self.xmin
			y[1] = x[1]
			y[2] = self.zmin
		elif (
			(dolfin.near(x[1], self.ymax, self.tol) and dolfin.near(x[2], self.zmax, self.tol))
			or (dolfin.near(x[1], self.ymax, self.tol) and dolfin.near(x[2], self.zmin, self.tol))
			or (dolfin.near(x[1], self.ymin, self.tol) and dolfin.near(x[2], self.zmax, self.tol))
		):
			y[0] = x[0]
			y[1] = self.ymin
			y[2] = self.zmin
