...
for loads in loads_lst:
	...
	for inverse in inverse_lst:
		...
		U, phis, dV = dmech.runs.RivlinCube_PoroHyperelasticity(
			dim=dim,
			inverse=inverse,
	...
	U_tot_norm = (dolfin.assemble(dolfin.inner(U_tot, U_tot) * dV) / 2 / dolfin.assemble(dolfin.Constant(1) * dV)) ** (
		1 / 2
	)
	print("U_tot_norm:", U_tot_norm)

	assert U_tot_norm < 1e-2, "Warning! Did not find the initial geometry. Aborting."
