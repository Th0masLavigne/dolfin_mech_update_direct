...
mat_params = {"alpha": 0.16, "gamma": 0.5, "c1": 0.2, "c2": 0.4, "kappa": 1e2, "eta": 1e-5}
		...
		for known_porosity in known_porosity_lst:
			...
			for init_porosity in init_porosity_lst:
				...
				for scaling in scaling_lst:
					...
					if dim == 3:
						load_lst += ["p_boundary_condition0"] if (inverse) else ["p_boundary_condition"]
					for load in load_lst:
						...
						dmech.runs.RivlinCube_PoroHyperelasticity(
							dim=dim,
							inverse=inverse,
